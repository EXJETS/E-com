...
export default function Navbar() {
  ...
  const navBg = scrolled
    ? "bg-white/96 backdrop-blur-md border-b border-[var(--border)] shadow-sm"
    : "bg-transparent";
    ...
    <>
      ...
      >
        <nav className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between gap-8">
          ...
          <Link
            href="/"
            className={`text-lg font-bold tracking-tight flex-shrink-0 transition-colors duration-300 ${iconColor}`}
          >
            NexHome
          </Link>
          ...
          <div className="hidden md:flex items-center gap-6 flex-1 justify-center">
            ...
            >
              <button
                className={`nav-link flex items-center gap-1 transition-colors duration-300 ${linkColor}`}
              >
                All Categories
                <svg
              ...
              {/* Mega dropdown */}
              <div
                className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[660px] bg-white border border-[var(--border)] rounded-xl shadow-2xl transition-all duration-200 ${
                    ...
                    {featured.map((p) => (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-semibold text-[var(--dark)] line-clamp-2 leading-snug">
                            {p.name}
                          </p>
          ...
          <div className="flex items-center gap-0.5 flex-shrink-0">
            <button
              className={`icon-btn transition-colors duration-300 ${iconColor} hover:bg-white/10`}
              aria-label="Search"
            ...
            <button
              onClick={toggleCart}
              className={`icon-btn relative transition-colors duration-300 ${iconColor} hover:bg-white/10`}
              aria-label="Open cart"
            >
              <ShoppingBag className="w-[18px] h-[18px]" />
              {totalItems > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[var(--accent)] text-white text-[9px] font-bold rounded-full flex items-center justify-center leading-none">
      ...
      {/* ── Mobile slide panel ─────────────────────────────────────── */}
      <div
        className={`fixed right-0 top-0 h-full w-[300px] bg-white shadow-2xl z-[60] flex flex-col transition-transform duration-300 md:hidden ${
          menuOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
          <Link href="/" className="text-lg font-bold text-[var(--dark)]" onClick={() => setMenuOpen(false)}>
            NexHome
          </Link>
          <button
            onClick={() => setMenuOpen(false)}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-6 flex flex-col gap-1">
          ...
          <div className="mt-5 pt-5 border-t border-[var(--border)]">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--muted)] px-3 mb-2">
              Categories
            </p>
            {collections.map((c) => (
              <Link
                ...
                href={`/collections/${c.id}`}
                onClick={() => setMenuOpen(false)}
                className="flex items-center justify-between px-3 py-2.5 rounded-lg text-sm text-[var(--text)] hover:bg-[var(--light)] hover:text-[var(--dark)] transition-colors"
              >
                <span>{c.name}</span>
